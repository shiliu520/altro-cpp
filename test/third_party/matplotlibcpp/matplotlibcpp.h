...
#else
// C++14 及以下：必须带消息 + 静态成员类内声明+类外定义（或 constexpr 优化）
static_assert(sizeof(long long) == 8, "long long must be 8 bytes (required for NPY_INT64)");
template <>
struct select_npy_type<long long> {
    static constexpr NPY_TYPES type = NPY_TYPES::NPY_INT64; // C++11+ 支持 constexpr 类内初始化
};

static_assert(sizeof(unsigned long long) == 8, "unsigned long long must be 8 bytes (required for NPY_UINT64)");
template <>
struct select_npy_type<unsigned long long> {
    static constexpr NPY_TYPES type = NPY_TYPES::NPY_UINT64;
};
#endif
